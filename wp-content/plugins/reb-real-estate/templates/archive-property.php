    ...
    <form class="reb-filter-form" method="get">
        <div class="reb-fields">
            <label>
                ...
                <input type="number" name="price_min" value="<?php echo esc_attr(isset($_GET['price_min']) ? sanitize_text_field(wp_unslash($_GET['price_min'])) : ''); ?>">
            </label>
            <label>
                <span><?php esc_html_e('Max Price', 'reb-real-estate'); ?></span>
                <input type="number" name="price_max" value="<?php echo esc_attr(isset($_GET['price_max']) ? sanitize_text_field(wp_unslash($_GET['price_max'])) : ''); ?>">
            </label>
            <label>
                ...
                <select name="property_type">
                    ...
                    <?php foreach ($types as $type) : ?>
                        <option value="<?php echo esc_attr($type->slug); ?>" <?php selected(isset($_GET['property_type']) ? sanitize_text_field(wp_unslash($_GET['property_type'])) : '', $type->slug); ?>>
            ...
            <label>
                ...
                <select name="property_location">
                    ...
                    <?php foreach ($locations as $location) : ?>
                        <option value="<?php echo esc_attr($location->slug); ?>" <?php selected(isset($_GET['property_location']) ? sanitize_text_field(wp_unslash($_GET['property_location'])) : '', $location->slug); ?>>
            ...
            <label class="reb-check">
                <input type="checkbox" name="include_sold" value="1" <?php checked(!empty($_GET['include_sold'])); ?>>
